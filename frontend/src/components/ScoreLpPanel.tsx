...
export function ScoreLpPanel({ p1, p2, history, activePlayer, hapticsEnabled, onChange, onSoundLp }: { p1: number; p2: number; history: LpLog[]; activePlayer: PlayerId; hapticsEnabled: boolean; onChange: (player: PlayerId, amount: number) => void; onSoundLp: (player: PlayerId, amount: number) => void }) {
  ...
  return <section className="score-player-area">
    <ScoreDuelistPanel player="p1" lp={p1} last={p1Last} activePlayer={activePlayer} hapticsEnabled={hapticsEnabled} onAdjust={amount => onChange('p1', amount)} onCustom={() => openCustom('p1')} onSoundLp={onSoundLp} />
    <ScoreDuelistPanel player="p2" lp={p2} last={p2Last} activePlayer={activePlayer} hapticsEnabled={hapticsEnabled} onAdjust={amount => onChange('p2', amount)} onCustom={() => openCustom('p2')} onSoundLp={onSoundLp} />
