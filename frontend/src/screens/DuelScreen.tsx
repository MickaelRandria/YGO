...
export function DuelScreen({ duel, scoreMode = false, onNext, onEndTurn, onSelectPhase, onToggleTimer, onLp, onMove, onAddEvent, onOpenLookup, onRematch, onNewDuel }: { duel: DuelState; scoreMode?: boolean; onNext: () => void; onEndTurn: () => void; onSelectPhase: (phase: Phase) => void; onToggleTimer: () => void; onLp: (player: PlayerId, amount: number) => void; onMove: (player: PlayerId, from: Zone, to: Zone, id: number) => void; onAddEvent: (player: EventPlayer, content: string) => void; onOpenLookup: () => void; onRematch: () => void; onNewDuel: () => void }) {
  ...
  const [focusedPlayer, setFocusedPlayer] = useState<PlayerId>(duel.activePlayer)
  ...
  useEffect(() => { if (scoreMode) setFocusedPlayer(duel.activePlayer) }, [duel.activePlayer, scoreMode])
  ...
  const common = scoreMode
    ? <><PhaseBar compact turn={duel.turn} player={duel.activePlayer} phase={duel.phase} turnStartedAt={duel.turnStartedAt} timerEnabled={duel.timerEnabled} onNext={onNext} onEndTurn={onEndTurn} onSelectPhase={onSelectPhase} onToggleTimer={onToggleTimer} /><ScoreLpPanel p1={duel.players.p1.lp} p2={duel.players.p2.lp} history={duel.lpHistory} activePlayer={duel.activePlayer} focusedPlayer={focusedPlayer} onFocus={setFocusedPlayer} onChange={onLp} /><div className="duel-quick-actions"><button onClick={onOpenLookup}><BookOpenCheck size={16} /> Vérifier une carte</button><button onClick={() => setShowManual(true)}><Flag size={16} /> Terminer le duel</button></div></>
    : <><PhaseBar turn={duel.turn} player={duel.activePlayer} phase={duel.phase} turnStartedAt={duel.turnStartedAt} timerEnabled={duel.timerEnabled} onNext={onNext} onEndTurn={onEndTurn} onSelectPhase={onSelectPhase} onToggleTimer={onToggleTimer} /><LpPanel p1={duel.players.p1.lp} p2={duel.players.p2.lp} history={duel.lpHistory} activePlayer={duel.activePlayer} onChange={onLp} /><div className="duel-quick-actions"><button onClick={onOpenLookup}><BookOpenCheck size={16} /> Vérifier une carte</button><button onClick={() => setShowManual(true)}><Flag size={16} /> Terminer le duel</button></div></>
