...
import { DuelCreature } from '../components/DuelCreature'
...
export function DuelScreen({ duel, scoreMode = false, onNext, onEndTurn, onSelectPhase, onToggleTimer, onLp, onMove, onAddEvent, onOpenLookup, onRematch, onNewDuel }: { duel: DuelState; scoreMode?: boolean; onNext: () => void; onEndTurn: () => void; onSelectPhase: (phase: Phase) => void; onToggleTimer: () => void; onLp: (player: PlayerId, amount: number) => void; onMove: (player: PlayerId, from: Zone, to: Zone, id: number) => void; onAddEvent: (player: EventPlayer, content: string) => void; onOpenLookup: () => void; onRematch: () => void; onNewDuel: () => void }) {
  ...
  const phaseProps = { turn: duel.turn, player: duel.activePlayer, phase: duel.phase, turnStartedAt: duel.turnStartedAt, timerEnabled: duel.timerEnabled, hapticsEnabled, onNext, onEndTurn, onSelectPhase, onToggleTimer, onToggleHaptics: () => setHapticsEnabled(current => !current) }
  const common = scoreMode
    ? <><PhaseBar compact {...phaseProps} /><MomentumGauge p1={duel.players.p1.lp} p2={duel.players.p2.lp} /><ScoreLpPanel p1={duel.players.p1.lp} p2={duel.players.p2.lp} history={duel.lpHistory} activePlayer={duel.activePlayer} hapticsEnabled={hapticsEnabled} onChange={onLp} /><div className="duel-quick-actions"><button onClick={onOpenLookup}><BookOpenCheck size={16} /> Vérifier une carte</button><button onClick={() => setShowManual(true)}><Flag size={16} /> Terminer le duel</button></div></>
