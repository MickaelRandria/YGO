...
export function DeckBuilder({ deck, player, onSave, onBack }: { deck: Deck; player: string; onSave: (cards: Card[]) => void; onBack: () => void }) {
  ...
  return <main className="screen deck-builder">
    ...
    <div className="mode-switch"><button className={mode === 'scan' ? 'active' : ''} onClick={() => setMode('scan')}>📸 Scanner</button><button className={mode === 'search' ? 'active' : ''} onClick={() => setMode('search')}>🔍 Recherche</button></div>
